...
@Injectable()
export class EthereumLogsListener implements OnModuleInit, OnModuleDestroy {
  ...
  constructor() {
    ...
    // console.log("webSocketObject", this.webSocketObject);
  ...
  async onModuleInit() {
    this.webSocketObject.onopen = async () => {
        ...
        let walletArray = ['0xd07AF8938673085F09efF3A559752B360883B61b', '0x41762AcEa5818fcA403b8A7855dF30f9DfaaD583'];
        //console.log('walletArray Before', walletArray);

        let walletArrayEncoded = [];
        //Update wallet value to the encoded string for Topics
        walletArray.forEach(wallet => { walletArrayEncoded.push(this.web3Object.eth.abi.encodeParameter('address', wallet)) });
